...
class SoftwaremediaController extends Controller
{
    ...
    public function index()
    {
       ...
       if($temp9 != null){
        if(strpos($temp9,"+") !== false){
            $arraypdf = (explode("+",$temp9));
            foreach($arraypdf as $arr){
                $path_arrray[] = $arr;   
            }
    ...
    public function update(SoftwaremediaRequest $request, $id)
    {
        ...
        $data->software_id = $request->input('software_id');
            ...
            if(strpos($value, "+") !== false){
             ...
             $filename = "screenshot_".rand().".".$screenshots->extension();
             $screenshots->storeAs('public/screenshots/',$filename);
             $data->screenshots = Config::get('app.url').'/storage/screenshots/'.$filename;
            } 
    ...
    {
       //fetch from DB and delete in database as well as  storage
